...
#[deprecated = "create a RegistryReq::Project and use .index_url()?.cache_path()?"]
pub fn registry_path(manifest_path: &Path, registry: Option<&str>) -> Result<PathBuf> {
    Ok(RegistryReq::project(registry, manifest_path)
        .index_url()?
        .cache_path()?)
}

#[deprecated = "create a RegistryIndex can use .cache_path()?"]
pub fn registry_path_from_url(registry: &Url) -> Result<PathBuf> {
    RegistryIndex(registry.clone()).cache_path()
}
...
impl RegistryIndex {
    /// Attempts to parse a RegistryIndex as a URL from a string.
    ///
    /// Returns an InvalidCargoConfig error if the URL is invalid.
    ...
    /// Returns the local filesystem path containing the registry's cached crates
    ...
    /// Returns the cargo-internal short-name of the registry
    pub fn short_name(&self) -> String {
        ...
        let ident = self.0.host_str().unwrap_or("").to_string();
...
impl std::fmt::Display for RegistryIndex {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        // just passthrough to Url's Display impl
        std::fmt::Display::fmt(&self.0, f)
    }
}
impl Default for RegistryIndex {
    /// Returns the index for `crates.io`
    fn default() -> Self {
        RegistryIndex::parse(CRATES_IO_INDEX).unwrap()
    }
}
impl AsRef<Url> for RegistryIndex {
    fn as_ref(&self) -> &Url {
        &self.0
    }
}
impl From<Url> for RegistryIndex {
    fn from(url: Url) -> RegistryIndex {
        RegistryIndex(url)
    }
}
impl From<RegistryIndex> for Url {
    fn from(ri: RegistryIndex) -> Url {
        ri.0
    }
}
...
impl<'a> RegistryReq<'a> {
    ...
    pub fn index_url(&self) -> Result<RegistryIndex> {
        ...
        fn lookup_registry(
            ...
            let mut source = match registry_name {
        ...
        let registry_index: Url = match self {
        ...
        Ok(RegistryIndex(registry_index))
    }
    ...
    #[deprecated = "call `.index_url()?.cache_path()?` instead"]
    pub fn cache_path(request: &RegistryReq) -> Result<PathBuf> {
        Ok(request.index_url()?.cache_path()?)
    }
}
...
#[deprecated = "create a RegistryReq and call .index_url()? on it"]
pub fn registry_url(manifest_path: &Path, registry: Option<&str>) -> Result<Url> {
    Ok(RegistryReq::project(registry, manifest_path).index_url()?.0)
}

#[deprecated = "use RegistryIndex::short_name()"]
fn short_name(registry: &Url) -> String {
    RegistryIndex(registry.clone()).short_name()
}

#[cfg_attr(target_pointer_width = "64", test)]
fn test_short_name() {
    fn test_helper(url: &str, name: &str) {
        let url = RegistryIndex::parse(url).unwrap();
        assert_eq!(url.short_name(), name);
    }
    test_helper(CRATES_IO_INDEX, "github.com-1ecc6299db9ec823");
}
