...
fn cargo_home() -> Result<PathBuf> {
    let default_cargo_home = dirs::home_dir()
        .map(|x| x.join(".cargo"))
        .ok_or_else(|| Error::ExpectedValue)?;
...
pub fn registry_url(manifest_path: &Path, registry: Option<&str>) -> Result<Url> {
    ...
    fn read_config(registries: &mut HashMap<String, Source>, path: impl AsRef<Path>) -> Result<()> {
        ...
        let config =
            toml::from_slice::<CargoConfig>(&content).map_err(|_| Error::Invalid("Invalid cargo config".into()))?;
    ...
    let mut source = match registry {
        ...
        Some(r) => registries
            .remove(r)
            .ok_or_else(|| Error::NotFound(r.to_string()))?,
    };
    ...
    while let Some(replace_with) = &source.replace_with {
        source = registries
            .remove(replace_with)
            .ok_or_else(|| Error::NotFound(replace_with.to_string()))?;
    }

    let registry_url = source
        .registry
        .and_then(|x| Url::parse(&x).ok())
        .ok_or_else(|| Error::Invalid("Invalid cargo config".into()))?;
